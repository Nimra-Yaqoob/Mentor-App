...
class MentorsList extends StatelessWidget {
  ...
  const MentorsList({Key? key, required this.searchQuery}) : super(key: key);
    ...
    return SizedBox(
      ...
      child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance
            .collection('mentors')
            .orderBy('reviewCount', descending: true) // Sort by reviewCount
            .snapshots(),
  ...
  Future<void> _toggleEnrollment() async {
    ...
    if (isEnrolled) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Successfully unenrolled')),
      );
    } else {
      // Enroll logic
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('This mentor has $enrollmentCount mentees enrolled.'),
          actions: [
            TextButton(
              child: Text('OK'),
              onPressed: () {
