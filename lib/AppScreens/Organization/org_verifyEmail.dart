...
class VerifyEmailScreen extends StatelessWidget {
  ...
  Future<void> _checkEmailVerified(BuildContext context) async {
    ...
    if (user == null) {
      _showErrorSnackBar(context, "No user is currently signed in.");
      return;
    }
    ...
    if (user.emailVerified) {
      ...
      Get.to(() => SuccessScreen(
            subtitle:
                "Congratulations! Your email activation was successful. Please note that admin verification is still pending, which may take a few hours. You will receive a confirmation email promptly once the verification is complete.",
            image: 'image/Verification.gif',
            title: 'Your Account Successfully Created!',
            onPressed: () => Get.to(() => const OrgnizationLoginScreen()),
          ));
    } else {
  ...
  Future<void> _deleteUserFromFirestore(BuildContext context) async {
    try {
      await FirebaseFirestore.instance
          ...
          .doc(userId)
          .delete();
      Get.offAll(() => OrgnizationLoginScreen());
    } catch (e) {
          ...
          child: FutureBuilder<String>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return Center(
                    child: Text('No email found or Document does not exist'));
              }
